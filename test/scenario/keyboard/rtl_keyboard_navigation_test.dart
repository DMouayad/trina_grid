...
void main() {
  group('RTL Keyboard Navigation', () {
    ...
    Future<void> buildGrid(
        WidgetTester tester, TextDirection textDirection) async {
    ...
    testWidgets('Home/End keys should work correctly in RTL mode',
      ...
      // Pressing Home should move to the visually leftmost cell (column2 in RTL)
      await tester.sendKeyEvent(LogicalKeyboardKey.home);
      await tester.pumpAndSettle();
      expect(stateManager.currentColumn?.field, 'column2');

      // Pressing End should move to the visually rightmost cell (column0 in RTL)
    ...
    testWidgets(
        'Tab navigation with moveToNextOnEdge should work correctly in RTL mode',
        (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Material(
      ...
      // Start at the last cell of the first row (visually leftmost in RTL)
      stateManager.setCurrentCell(rows[0].cells['column2'], 0);
      await tester.pumpAndSettle();
      ...
      // Pressing tab should move to the first cell of the next row
      await tester.sendKeyEvent(LogicalKeyboardKey.tab);
      await tester.pumpAndSettle();
      expect(stateManager.currentRowIdx, 1);
      expect(stateManager.currentColumn?.field,
          'column0'); // First cell of next row

      // Start at the first cell of the second row (visually rightmost in RTL)
      stateManager.setCurrentCell(rows[1].cells['column0'], 1);
      await tester.pumpAndSettle();

      // Pressing shift+tab should move to the last cell of the previous row
      await tester.sendKeyDownEvent(LogicalKeyboardKey.shift);
      await tester.sendKeyEvent(LogicalKeyboardKey.tab);
      await tester.sendKeyUpEvent(LogicalKeyboardKey.shift);
      await tester.pumpAndSettle();
      expect(stateManager.currentRowIdx, 0);
      expect(stateManager.currentColumn?.field,
          'column2'); // Last cell of previous row
    });

    testWidgets('Enter key navigation should work correctly in RTL mode',
        (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Material(
